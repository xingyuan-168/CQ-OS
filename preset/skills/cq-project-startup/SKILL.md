...
**证据不足时不得伪称已评估**：Research 若经 `web_search` 只获得摘要与链接、无法打开网页正文核实 License、仓库活跃度、维护状态等关键字段，必须在报告中明确标记“待获取权威源”，不得伪造“已评估”。此类缺口回流 Core，由 Core 决定是否补充人工/离线核验；证据不充分前，对应的开源结论标记为“暂缓/待核验”，不进入最终技术决策。
...
角色**禁止直接向用户提问**（`ask_user_question` 已收紧为 Core 专属）。当角色在分析中出现需要用户确认的澄清项、未决问题或歧义时：

1. 角色通过 `report` 向 Core 提出明确的澄清请求（说明问题、为何需要确认、候选选项）。
2. Core 收到后，判断是否需要用户决策；如需，由 Core 调用 `ask_user_question` 向用户提问。
3. 用户回答后，Core 将决策/确认结果作为后续任务的输入回传给对应角色。

角色不得用“已在 prompt 里说基于上一步”代替真实结果传递；澄清请求必须通过 `report` 上报，由 Core 统一触发 Human Gate。
